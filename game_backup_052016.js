...
const LEVELS = [
    {
        ...
        width: 4500, // Expanded
        hearts: 30,
        requiredHearts: 20, // User Request
        enemies: 6,
        checkpoints: [1500, 3000],
        platforms: [
            // SAFE FLOOR (Grass/Dirt) - Height 600
            [0, 420, 2000, 600],
            [2200, 420, 2300, 600],
            // Upper platforms
            [400, 300, 200, 30], [700, 250, 200, 30], [1000, 320, 200, 30],
            [1400, 300, 200, 30], [1700, 250, 200, 30], [2000, 350, 200, 30], // Gap bridge
    ...
    },
    {
        id: 2,
        name: "Dikenli Patika",
        width: 3000, // COMPACT - Guaranteed visibility
        hearts: 30,  // Exact 30 (User Request)
        requiredHearts: 20, // Exact 20 (User Request)
        enemies: 5,
        checkpoints: [1400],
        spikes: [
            [500, 420, 3],
        ...
        platforms: [
            ...
            [2200, 420, 700, 600],   // END: 2200-2900
        ],
        finalPlatform: [2950, 300, 200, 30], // 50px before boundary!
    },
    {
        id: 3,
        name: "Sakura Zirvesi",
        ...
        checkpoints: [1400],
        spikes: [
            // Death pits under moving platform gaps
            [1050, 600, 8], // Under gap 1
            [1150, 600, 8],
            [2050, 600, 8], // Under gap 2  
            [2150, 600, 8],
        ],
        platforms: [
            [0, 450, 800, 600],      // START platform
            [1400, 450, 800, 600],   // MID platform
            [2400, 450, 600, 600],   // END platform
        ],
        movingPlatforms: [
            // Chain 1: 1000 -> 1800 (Gap 800)
            // Start lower (380) so jump from 450 is easy
            { x: 1000, y: 380, w: 150, h: 30, moveX: 0, moveY: -80, duration: 2000 },
            { x: 1250, y: 350, w: 150, h: 30, moveX: 0, moveY: 80, duration: 2500 },

            // Chain 2: 2600 -> 3200 (Gap 600)
            // Start lower (380)
            { x: 2250, y: 380, w: 150, h: 30, moveX: 0, moveY: -80, duration: 2000 },
            { x: 3000, y: 250, w: 150, h: 30, moveX: 0, moveY: 150, duration: 2500 },

            // Final Ascent to Senam (at 3600)
            { x: 3500, y: 300, w: 150, h: 30, moveX: 0, moveY: -100, duration: 1500 }
        ],
        finalPlatform: [2900, 280, 300, 30], // 100px safety margin!
    }
];
...
function goBack() {
    window.location.href = "valentine-storybook.html";
}
...
let playerLives = 10; // Increased for testing (User request)
let heartsTotal = 0;
let frozen = false;
...
function startNewRun(nextIndex) {
    ...
    const realCount = placeHeartsOnPlatforms(hearts, platforms, heartsTotal, currentLevel.spikes);

    // Update goal if fewer hearts placed (avoid unwinnable state)
    if (realCount < heartsTotal) {
        // We only care about ratio, but HUD uses heartsTotal. 
        // Let's keep total same but ensure required is reachable?
        // Actually, safer to update total for HUD consistency.
        heartsTotal = realCount;
        currentLevel.requiredHearts = Math.ceil(heartsTotal * 0.8);
    }
...
function startNewRun(nextIndex) {
    ...
    if (levelIndex === 0) playerLives = 10; // Increased for testing
    ...
    if (heartsCollected >= required) {
        ...
        sceneRef.time.delayedCall(2000, () => {
            ...
            if (levelIndex < LEVELS.length - 1) {
                ...
                victoryDesc.textContent = `Bütün bölümler bitti! Kalpler: ${heartsCollected}/${heartsTotal} 💖`;
...
function placeEnemies(group, platformGroup, count) {
    ...
    // Filter platforms:
    // 1. Large enough (width >= 300)
    // 2. NOT in spawn zone (x < 500)
    // 3. NOT in final zone (x > levelWidth - 500)
    const validPlats = plats.filter(p => {
        const platCenterX = p.x;
        return p.width >= 300 && platCenterX > 500 && platCenterX < (levelWidth - 500);
    });

    if (validPlats.length === 0) return;

    let placed = 0;
    // Try to place 'count' enemies
    for (let i = 0; i < count; i++) {
        // Pick random valid platform
        const plat = validPlats[Math.floor(Math.random() * validPlats.length)];

        // Random x within platform bounds (padding 50px)
        const minX = plat.x - (plat.width / 2) + 50;
        const maxX = plat.x + (plat.width / 2) - 50;
        const x = Phaser.Math.Between(minX, maxX);

        // Spawn slightly above so they fall to surface (Gravity enabled now!)
        const y = plat.y - (plat.displayHeight / 2) - 50;

        const enemy = group.create(x, y, "enemy");
        // Origin (0.5, 1) means y is at the feet
        enemy.setOrigin(0.5, 1);
        // Sync physics body
        enemy.body.setSize(30, 30);
        enemy.body.setOffset(1, 2);
        // Important: bounce 0 to stay flat
        enemy.setBounce(0);
        enemy.setDragX(100); // Friction

        // Assign patrol limits
        enemy.setData("minX", minX);
        enemy.setData("maxX", maxX);
        enemy.setData("speed", ENEMY_SPEED);
        // CRITICAL FIX: Initialize state correctly
...
// HEART DISTRIBUTION    //
//////////////////////////
// [Modified] Accepts spikeData to exclude hearts from spike zones
function placeHeartsOnPlatforms(heartGroup, platformGroup, count, spikeData) {
    const plats = platformGroup.getChildren();
    ...
    if (spikeData) {
        spikeData.forEach(s => {
            ...
            // spike width 32
            const minX = s[0];
            const maxX = s[0] + s[2] * 32;
            exclusions.push({ min: minX, max: maxX });
        });
    }

    // Algorithm: Distribute hearts to platforms weighted by width
    let totalWidth = 0;
    const infos = [];
    plats.forEach(p => {
        const w = p.displayWidth;
        totalWidth += w;
        infos.push({ p, w, hearts: 0 });
    });

    // Assign hearts proportional to width
    let assigned = 0;
    for (let info of infos) {
        const share = Math.floor(count * (info.w / totalWidth));
        info.hearts = share;
        assigned += share;
    }
    ...
    let idx = 0;
    while (remaining > 0) {
        infos[idx % infos.length].hearts++;
        idx++;
        remaining--;
    }

    // Place them evenly (skipping spikes)
    let actualPlaced = 0;
    for (let info of infos) {
        if (info.hearts <= 0) continue;
        const p = info.p;
        const availableW = info.w - 60; // 30px padding
        const step = availableW / (info.hearts + 1);

        for (let k = 0; k < info.hearts; k++) {
            const x = p.body.left + 30 + step * (k + 1);

            // Check exclusion
            let blocked = false;
            for (let ex of exclusions) {
                // simple interval overlap check (heart width ~30)
                if (x + 10 > ex.min && x - 10 < ex.max) {
                    blocked = true;
                    break;
                }
            }
            if (blocked) continue; // SKIP this heart

            // Height adjusted: closer to ground (-22)
            const y = p.body.top - 22;
            const h = heartGroup.create(x, y, "heart");
            // Set circle hitbox for easier collection
            h.body.setCircle(20);
